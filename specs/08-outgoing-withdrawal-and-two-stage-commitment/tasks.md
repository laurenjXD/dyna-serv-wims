...
- [ ] **Added 2026-08-28**: Implement `lib/parsers/dra-parser.ts` parsing engine and `DraImportModal.tsx` UI component, with unit tests for Excel (`.xlsx`, `.csv`) and PDF (`.pdf`) DRA document parsing. Pre-fills Pick List draft lines and matches against available inventory balances prior to Stage 1 commitment.
