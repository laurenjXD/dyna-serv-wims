...
- [x] **B.9 Generate Drizzle migration** — `supabase/migrations/0034_vmi_billing_tables.sql` (generated against a scoped throwaway config since this repo's `drizzle-kit` journal has been stale/hand-authored since migration 0004 — see the migration's own file history; SQL verified equivalent to a clean generate).
...
- [x] **D.4 Implement recurring-fee aggregation** — File: `lib/billing/vmi-recurring-fee-aggregation.ts`. Sums active `vmi_recurring_fee_lines`, with `manpower` requiring an hours-logged entry for the period, sourced from `vmi_manpower_hours_log` (B.12) — `hours × manpower_rate_per_hour`, converted to USD; `$0` (never an error, still present in the result) when no matching row exists for the period. Does not join `vmi_permits` — the LOA/permit sync is a UI-layer concern (E.4), not this aggregation's job. 16/16 tests passing, full suite 1763/1763, zero failures.
- [x] **D.5 Implement forex rate locking** — File: `lib/billing/vmi-forex-lock.ts`. Fetch `forex_rates` where `effective_date = generation_date` (exact match only, no nearest-date fallback). Block period close with a clear error naming the missing date if absent. 10/10 tests passing (part of the 21-test combined suite with D.7).
- [x] **D.6 Implement SOA running-balance calculation** — File: `lib/billing/vmi-soa-balance.ts`. `soa_opening_balance_usd` = the party's most recent non-voided prior period's `soa_closing_balance_usd`, ordered by `period_end_date` (0 if none exists — first period, or all prior periods voided). `soa_payments_applied_usd` = `SUM(vmi_payments WHERE type='payment' AND applied_to_period_id = this period)`; `credit_memo`/`adjustment` types excluded. `soa_closing_balance_usd = opening + billing_statement_total_usd - payments_applied`, deliberately not clamped at 0 (unlike the statement total — a legitimate credit/overpayment can make this negative). 23/23 tests passing, full suite 1807/1807, zero failures.
- [x] **D.7 Implement period-number generation** — File: `lib/billing/vmi-period-number.ts`. Format `VMI-{YYYY}-{MM}-{PARTY_CODE}` (corrections: `-R{N}`, derived from the highest existing correction suffix for the same base + 1, gap-robust). Verifies uniqueness before returning (backstopped by the DB's real UNIQUE constraint on `period_number`). 11/11 tests passing (part of the 21-test combined suite with D.5). Full suite 1784/1784, zero failures.
- [x] **D.8 Implement the period-close command** — File: `lib/billing/vmi-period-close.ts`. Runs D.1-D.7 (plus a new `sumVmiStorageCharge` step — that specific sum had no owning sibling module, per the design.md §2.6 clarification added 2026-08-20), validates no existing non-voided period for the same party/month, computes `billing_statement_total_usd` (clamped at 0), inserts `vmi_billing_periods` with `status = 'draft'`. 20/20 tests passing, full suite 1827/1827, zero failures. This closes out Task Group D except D.9-D.11, which are blocked on `04`'s not-yet-built PDF artifact pipeline.
