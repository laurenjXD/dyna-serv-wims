...
**Current status**: only the spec *structure* exists right now — no spec has reached `Approved`. `01-core-data-model` is the critical path: it blocks every other spec, and it blocks resuming the project-setup/DB-schema work that was paused specifically because it started before this process existed. Check `specs/00-steering/gantt-mapping.md` for the live status of every spec against the delivery timeline before assuming anything is further along than it is.
