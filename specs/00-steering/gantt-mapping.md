...
| 2.5 | Inventory picking and quantity verification | `08-outgoing-withdrawal-and-two-stage-commitment` | **Approved** (2026-08-06 direct post-picking dispatch amendment, pre-dispatch inspection removed; 2026-08-24 dispatch-time barcode-scanning amendment, both approvals granted in conversation) | In Progress — Stage 2 dispatch (`dispatchPickList`) completed 2026-08-20 after being found genuinely incomplete (placeholder transaction data, no balance decrement, no commitment execution); full fix + 3 review-confirmed findings (TOCTOU race, missing RLS UPDATE policies, AR document contract) all resolved and verified. Further hardened 2026-08-22 (`billing/trading` merge): `SELECT ... FOR UPDATE` row lock plus a CAS-guarded balance update closing a cross-pick-list concurrent-dispatch race, real-Postgres-verified under genuine concurrency. Real-Postgres call-through-the-function integration testing still open for the dispatch-time barcode-scanning amendment specifically (`tasks.md` Integration tests section) |
...
| 3.6 | VMI CBM storage billing calculations | `12-vmi-billing` | **Approved** (2026-08-19 full rewrite — SOA billing pipeline model replaces the 2026-08-06 CBM-only period-average design; grounded in real June 2026 billing data; `vmi_contract_terms` is an effective-dated rate-version history, not a single mutable row, per the 2026-08-19 A.9 decision; all three documents and both sign-offs complete) | In Progress — Task Groups B and C both fully done (schema incl. B.11 amendment; movement-query engine, daily balance replay/storage charge, the full nightly `pg_cron`+Edge-Function pipeline, and the backfill utility — all implemented, tested, real-Postgres-verified where applicable). `08`'s Stage 2 placeholder-transaction gap (previously blocking VMI's OUT-side CBM) fixed and verified 2026-08-20 — see `08`'s own `tasks.md`. Task Group D (period close, four-document generation) not started, blocked in part on `04`'s PDF pipeline; E–H not started |
| 3.6a | Trading order pricing and margin calculations | `13-trading-orders-and-pricing` | **Approved** (2026-08-19 full rewrite — rate-card pricing model replaces the 2026-08-06 `trading_orders` lifecycle design entirely, no order entity; all three documents and both sign-offs complete; taxes/discounts/returns/post-dispatch-corrections policy explicitly still open per Task 1, to be resolved during Task 4 implementation) | In Progress — Task 2 (schema) and Task 3 (RBAC capabilities + default-deny RLS, `0036`/`0037`, both real-Postgres-verified) done; Task 4 mostly done (margin-calc math, price resolution, freeze command, override flow, margin/cost redaction — all implemented, tested, `rbac-rls-reviewer`/`offline-sync-reviewer`-passed; CRUD routes and `08` pick-list-generation wiring still open). Tasks 5–8 not started |
