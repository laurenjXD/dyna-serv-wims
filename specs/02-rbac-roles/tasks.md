...
All recommended defaults were accepted for the draft. The conditional defaults were resolved as follows: `1F = B` (party plus optional `flow_type` scope), `2F = B` (RBAC-specific security events because `01-core-data-model` does not define a general audit table), and `4F = B` (reviewed SQL helper functions for common policy checks). These selections must be formalized in `requirements.md` and `design.md` before this plan can be approved; this acceptance does not replace the sign-offs below.
