  ...
  return <section className="rounded bg-surface-white p-6 shadow-elevation-1">
    ...
    <div role="grid" aria-label={title} className="grid grid-flow-col grid-rows-7 auto-cols-[minmax(12px,1fr)] gap-0.5 overflow-x-auto pb-3">
      {visible.map((item) => <button key={item.date} type="button" role="gridcell" tabIndex={0} aria-label={`${item.date}: ${item.count} transactions`}
        ...
        className={`relative aspect-square min-h-3 min-w-3 rounded-sm ${levelClasses[heatmapLevel(item.count)]} focus:z-10 focus:outline-none focus:ring-2 focus:ring-brand-navy`}>
        {activeDate === item.date && <span role="status" className="absolute bottom-full left-0 z-20 mb-2 whitespace-nowrap rounded bg-on-surface px-2 py-1 font-body text-body-sm text-surface-white shadow-elevation-2">
