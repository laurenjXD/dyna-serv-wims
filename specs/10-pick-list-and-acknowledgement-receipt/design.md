...
Once generated, the PDF is the physical staging instruction and the parent pick list is dispatch-ready. The queue confirms generation and links to **View / PDF** and an explicit **Dispatch** action, while the operational QR scan occurs only at the direct Dispatch route.
