...
- Physical picking proceeds on the committed pick list; when all lines are accepted, the floor user or supervisor proceeds directly to `dispatch`.
...
- Aging or internal inspection is resolved by `11` before stock is selected for picking; outbound dispatch has no inspection branch.
...
8. After all pick/scan lines on a pick list are accepted, the floor user or supervisor SHALL proceed directly to `dispatch`; Stage 2 verifies scans, decrements inventory, releases reservations, writes the immutable `pick` transaction, and makes the priced `acknowledgement_receipt` available.
9. Stage 2 SHALL have no quality-check branch that blocks or reroutes dispatch.
...
- [ ] After all pick scans are accepted, dispatch is direct and has no pre-dispatch inspection route or state.
