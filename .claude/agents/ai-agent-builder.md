...
Note `product.md` names a fourth real role, **administrators** — but `15-ai-chatbot`'s current approved scope is exactly these three personas. Don't add an administrator persona or tool set on your own initiative because the role exists elsewhere in the system; that's a scope change to `15`, not an implementation detail, and needs its own spec approval first.

Every tool must respect the same `flow_type` (`'vmi'`/`'trading'`/`'supplies'`) partitioning the rest of the system enforces — a staff or supervisor query spanning flow types without the underlying data actually being flow-agnostic (e.g. a cross-flow stock lookup) is the same class of bug as a cross-flow billing leak elsewhere in the system, just surfaced through a chat tool instead of an API route.
