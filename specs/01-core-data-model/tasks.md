...
- [ ] **1. Drizzle ORM Schema Definitions**
  ...
  - [ ] Define the `master_inventory_tracking` and `lot_history_export` derived read-model contracts, including `lot_number` aging, the `displayed_item_code`/`item_code_is_provisional` flow-based item-code resolution, connected event identity, and financial projection separation (Req 16, Design 3.4)
  ...
  - [ ] Define `wrr_documents` and `wrr_items` tables in `lib/db/schema/wrr.ts` with `cipl_file_url`, `peza_number`, `commercial_invoice_no`, `ip_number`, `mawb_mbl_number` (Req 2.4, Design 1.2)
