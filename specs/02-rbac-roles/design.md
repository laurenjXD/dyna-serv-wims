...
| `lots` | `read` | `assigned_party` | `party_user` |
| `lot_location_balances` | `read` | `assigned_party` | `party_user` |
...
| `notifications` | `update_state` | `global` | `warehouse_staff`, `supervisor`, `administrator` |
| `notifications` | `update_state` | `assigned_party` | `party_user` |
...
`manage_rules`/`read_diagnostics`/`update_state` originate from
`14-notifications-and-alerts` requirements.md R6.1 and tasks.md Task Group
3's "add notification read/read-state/preferences/operations capabilities
to the canonical RBAC catalog" item. `notifications.read` itself is not
new at either scope — both the `global` row (`warehouse_staff`/
`supervisor`/`administrator`; staff aren't party-bound, and several
`R1-A` operational alerts are not party-scoped events) and the
`assigned_party` row (`party_user`, who must see only notifications tied
to their own party per design.md §5's authorization intersection) already
existed in this catalog before this amendment — this entry is listed here
only for traceability, not as a change. `update_state` (marking a
notification read/acknowledged/dismissed) mirrors `read`'s own scope
split: `global` for `warehouse_staff`/`supervisor`/`administrator`,
`assigned_party` for `party_user` — a recipient can only ever mutate their
own notification row (enforced by the query layer's/RLS's
`recipient_user_id = auth.uid()` predicate, the actual row-level boundary;
the capability grant only gates whether the mutation surface is reachable
at all). `manage_preferences` uses `global` scope kind with an
application/RLS-layer `user_id = auth.uid()` restriction, since a user
manages only their own preferences regardless of party scope, the same
self-row pattern already used for `user_profiles`.
...
- `parties`: All active internal users with `parties.read` (`global`) select all rows. Party users select the `parties` row for any party they hold an active, non-revoked `user_party_scopes` assignment for, **regardless of that assignment's `flow_type` value**: the predicate is a direct assignment-existence check — an active `user_party_scopes` row exists where `party_id` matches the target row's `id` and `user_id` matches the caller — not a call to `can_access_party_resource`. This must be evaluated through the same `SECURITY DEFINER`/`rbac_definer` mechanism as the other RLS helpers in §7.2 (an inline subquery against `user_party_scopes` inside this policy would itself be subject to `user_party_scopes`' own SELECT RLS at evaluation time, the same self-defeating-policy-recursion problem `rbac_definer`/`BYPASSRLS` exists to solve for the other helpers), not a bare inline `EXISTS`. The check is deliberately flow-agnostic and does not route through `can_access_party_resource`'s flow-matching logic at all, because `parties` rows are not themselves flow-partitioned data — a party is one record regardless of which flow(s) a user's assignment covers, unlike `wrr_documents`/`lots`/`pick_lists`, which genuinely are.

  **Corrected 2026-08-08 (`db-migration-verifier` and `rbac-rls-reviewer`, independently converged on the same finding while reviewing `supabase/migrations/0008_rls_policies.sql`)**: the previous formulation, `can_access_party_resource('parties', 'read', id, null)`, was a genuine logic bug, not merely an implementation-faithfulness question. `has_party_scope`'s null-`flow_type` matching (§3.2/§7.2) only matches an assignment row whose OWN `flow_type` is ALSO null (an all-flow grant); passing a literal `null` as the requested `flow_type` does not mean "match any flow." Under the old formulation, a party user legitimately scoped to one specific flow only (e.g. `flow_type = 'vmi'` — a normal administrative choice per §4.6, not an edge case) was denied all visibility into their own counterparty's `parties` row, even though that same user could already read that party's `wrr_documents`/`pick_lists`/`lots` rows — blocking the basic UI need of showing a party's own name/code alongside a document the user can already read. The corrected predicate above resolves this by checking assignment existence directly instead of through `can_access_party_resource`'s flow-matching semantics, which never applied to this non-flow-partitioned table in the first place. A future reader must not reintroduce the `can_access_party_resource(..., null)` formulation for this table. Administrators insert and update with `parties.manage`. No user-initiated delete policy; deactivation uses `is_active`. Party users receive no insert, update, or delete policy.
...
- `wrr_advance_notices` (**schema amendment, 2026-08-06** — table itself is a `01-core-data-model` addition not yet through its own `db-migration-verifier` pass; this RLS pattern is PROPOSED for `02`'s review, not self-authorized, per `22-parties-portal` requirements.md R11. **Revised 2026-08-06 in response to `rbac-rls-reviewer` findings B, C, D, E — the pattern below replaces the original single-paragraph version wholesale, not a patch on top of it.**): INSERT-only for `party_user` holding `shipment_labels.generate` (`assigned_party`), gated by a WITH CHECK clause with **four** independent conditions, all of which must pass:
  ...
  4. **Submitter-identity spoofing prevention (added 2026-08-06, `db-migration-verifier` pass-4 finding, closed pass 5)**: `submitted_by_user_id = auth.uid()`. Pass 4's live test proved that without this condition, the column added for §7.4a's self-review prohibition (finding immediately below) is trivially spoofable — a client can INSERT any arbitrary UUID into `submitted_by_user_id`, not just their own, which silently defeats the self-review check entirely (a dual-role account submits under a fabricated `submitted_by_user_id`, then confirms as staff, and the identity comparison never matches because the stored value was never actually the true submitter). This condition is not optional hardening; it is what makes the self-review comparison meaningful at all. Pass 5 confirmed the fix closes the spoofing path at the write itself (the spoofed INSERT is rejected outright, no row is ever created for a later confirm-side check to miss) without breaking the honest case.

  **Implementation note (pass-5 finding, not a security gap)**: `party_user` correctly has no SELECT policy on `wrr_advance_notices` (only internal staff get read access, via `receiving.view`). A client INSERT using Supabase's `.select()`/`RETURNING` pattern will therefore fail even on a fully valid, honest insert, because `RETURNING` implicitly requires the new row to also satisfy a SELECT policy. The server action implementing this INSERT must omit `.select()`/`RETURNING` for the `party_user` session — a plain INSERT with no return payload — rather than add a SELECT grant for the submitting party (which is out of scope and unnecessary; the confirming barcode/label content the party needs is already known client-side from the form they just submitted).
...
`party_user` correctly has no UPDATE policy on `wrr_advance_notices` (§7.4 above). Internal staff holding `receiving.view` (read pending advance notices) and the existing global `receiving.confirm` capability (confirm/reject/match action) perform the UPDATE through a **controlled `SECURITY DEFINER` function**, not a direct RLS UPDATE policy — this is the same defense-in-depth pattern already required of every other privileged function in §8: "a privileged function that only performed its side effects without its own authorization check would be a bypass path independent of whatever the calling code did correctly." Concretely, the function:

1. Independently re-verifies the caller holds `receiving.confirm` inside its own transaction, exactly as §8's opening paragraph requires of every admin/privileged function in this spec, rather than trusting that an upstream `requirePermission()` call already happened.
2. **Self-review prohibition (added 2026-08-06, `rbac-rls-reviewer` finding)**: independently verifies the caller's identity does not equal the `party_user` who submitted the `wrr_advance_notices` row being acted on. This is not implied by role separation alone — nothing in `02`'s role model (§4) makes a `party_user` assignment and an internal `warehouse_staff`/`supervisor` role grant mutually exclusive on the same `user_profiles` row, so a dual-role account could otherwise submit an advance notice as a party contact and confirm/reject it as staff with no check catching it. This mirrors §3.4's self-approval prohibition for `fifo_override.approve` (comparing the acting user against `requester_user_id`) — the same explicit, named, server-enforced pattern is required here, not assumed from organizational separation. The comparison field is the submitting party_user's `auth.uid()`, captured at INSERT time (no new column beyond what's already needed — `wrr_advance_notices` has no explicit submitter-user column today; add one, e.g. `submitted_by_user_id`, alongside this function's implementation, analogous to how `confirmed_by_user_id` already captures the confirming actor).
3. Performs the `wrr_advance_notices` status transition (`pending_review` → `confirmed`/`rejected`) and, on confirmation, the linkage to a staged `wrr_items` line, transactionally.
4. Sets `confirmed_at`/`confirmed_by_user_id` from the verified server-side actor, never a client-supplied value.

**Decision resolved 2026-08-06:** reuse the existing global `receiving.confirm` capability for the confirm/reject/match operation. `receiving.view` remains the review/read capability. The controlled function still independently checks `receiving.confirm`, the target state, the CIPL/matching invariants, and the self-review prohibition; no new role-name or ad-hoc capability check is introduced.
...
- [ ] Test that the back-office confirm/reject function (finding F, §7.4a) independently re-verifies `receiving.confirm` inside its own transaction rather than trusting an upstream `requirePermission()` call — using a control experiment that calls the function directly with a caller lacking the capability and confirms it is rejected. **Pending implementation of the controlled function; the capability decision is now resolved.**
- [x] **Added 2026-08-06, `rbac-rls-reviewer` self-review-prohibition finding, closed pass 5**: test that `submitted_by_user_id` cannot be spoofed at INSERT (WITH CHECK condition 4) and that the self-review prohibition it enables actually blocks a dual-role identity (active `party_user` scope + internal staff role on one account) from confirming a submission they made under their own true identity. **PASS, real-Postgres pass 4+5 (2026-08-06)**: pass 4 found the column was spoofable before condition 4 existed (a client could INSERT an arbitrary UUID, silently defeating the self-review check); pass 5 confirmed the fix rejects the spoofed INSERT outright — no row is ever created for a later confirm-side check to miss — while the honest case still succeeds and correctly triggers the self-review block in a simulated confirm. **Implementation note surfaced by this test, not a security gap**: a `party_user` INSERT must not use Supabase's `.select()`/`RETURNING` pattern (no SELECT policy exists for `party_user` on this table), or a fully valid insert will appear to fail.
