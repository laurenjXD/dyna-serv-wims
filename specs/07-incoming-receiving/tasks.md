...
Status: Approved
Updated: 2026-08-24 (Product Owner decision: batch putaway model adopted, per-unit model retired) — resolves the `fix-it-felix` merge contradiction flagged the same day (Task 6's former FLAGGED block). The batch putaway allocation model is now implemented (`commitWrrLine`, `lib/actions/receiving.ts`; `wrr_item_putaway_allocations`, migration `0032`) and is the sole authoritative `store`/`inspect`-line commit path. The 2026-08-20/21 per-unit model (`commitStoreUnit`, `commitInspectLine`, per-unit idempotency-key machinery) has been deleted from the codebase. See `specs/00-steering/revision-log.md`'s 2026-08-24 entry for the full account.
