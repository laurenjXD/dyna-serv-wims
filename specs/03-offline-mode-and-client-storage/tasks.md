...
- [x] Agree whether v1 supports foreground/reconnect sync only or Service Worker background wake-up as well. *(Resolved 2026-08-09: foreground/reconnect sync only. Queue flushes when user returns to app and browser reconnects. No Service Worker Background Sync API required. Migration to background sync is additive in v2. See revision-log.md.)*
...
- [x] Define whether rejected/conflicted operations are reviewed in a shared office surface or in owning feature screens. *(Resolved 2026-08-09: shared `/sync` office surface. Floor users see a "X failed — see Sync" shell banner; full detail at `/sync`. Owning feature screens show current committed state only, not the failed queue entry. See revision-log.md.)*
