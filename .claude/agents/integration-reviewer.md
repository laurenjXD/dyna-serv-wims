...
The concrete shared surfaces most seams run through, from `01-core-data-model`: `lots` (partitioned by `flow_type`), `lot_location_balances`, `inventory_commitments`/`inventory_commitment_lines` (lifecycle: `active` → `inspection_pending` → `executed`/`released`/`expired`/`cancelled`), `inventory_transactions`, `wrr_documents`/`wrr_items`/`wrr_inspection_logs`, `pick_lists`/`pick_list_items`, and the cross-cutting `audit_log`. Most real seam bugs in this system will be two features disagreeing about state on one of these tables, not about their own private tables.

What actually breaks at seams, concretely — check for these:
1. **Shared table, divergent assumptions.** Two features both write/read `inventory_transactions`, `lots`, or `inventory_commitments` — do they agree on what a given `movement_type`, `flow_type`, lot `status`, or `commitmentStatusEnum` value means, or did one feature invent a value the other doesn't handle?
