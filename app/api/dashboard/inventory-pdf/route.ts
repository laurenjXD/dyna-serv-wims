...
export async function GET() {
  ...
  function renderPageHeader(page: PDFPage, pageNum: number, totalPages: number, isFirstPage: boolean): number {
    if (logoImage) {
    ...
    page.drawText("Warehouse Operations & Master Inventory Executive Summary Report", { x: headerX, y: 526, size: 8.5, font: bold, color: MUTED });

    page.drawText(`GENERATED: ${nowStr}`, { x: 610, y: 542, size: 7.5, font: bold, color: INK });
    page.drawText(`STATUS: AUTHORIZED REAL-TIME TELEMETRY`, { x: 610, y: 528, size: 6.5, font: regular, color: MUTED });
    if (totalPages > 1) {
      page.drawText(`PAGE ${pageNum} OF ${totalPages}`, { x: 740, y: 554, size: 7, font: bold, color: MUTED });
    }

    if (isFirstPage) {
      // Top KPI Summary Cards on First Page
      const kpiY = 506;
      ...
      page.drawText("LIVE MASTER INVENTORY POSITIONS & BALANCES", { x: MARGIN, y: tableTitleY, size: 8.5, font: bold, color: ACCENT_NAVY });
      ...
      return colHeaderY - 20;
    } else {
      const colHeaderY = 506;
      let x = MARGIN;
      headings.forEach((heading, index) => {
        drawCell(page, heading, x, colHeaderY, widths[index], 20, bold, 6.5, index >= 4 && index <= 5 ? "right" : index === 2 || index === 6 ? "center" : "left", HEADER_FILL);
        x += widths[index];
  ...
  const ROWS_SUBSEQUENT_PAGE = 20;
  const totalPages = allItems.length <= ROWS_PAGE_1 ? 1 : 1 + Math.ceil((allItems.length - ROWS_PAGE_1) / ROWS_SUBSEQUENT_PAGE);

  let itemCursor = 0;
  let pageIndex = 0;

  while (itemCursor < allItems.length || pageIndex === 0) {
    pageIndex++;
    const isFirst = pageIndex === 1;
    const page = pdf.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
    let rowTop = renderPageHeader(page, pageIndex, totalPages, isFirst);

    const pageSize = isFirst ? ROWS_PAGE_1 : ROWS_SUBSEQUENT_PAGE;
    const pageItems = allItems.slice(itemCursor, itemCursor + pageSize);
    ...
    // On final page, render the signature footer
    if (pageIndex === totalPages || itemCursor >= allItems.length) {
      const footerY = 54;
