...
**Implementation status (added 2026-08-11):** AC 1/2 (camera scanner reachable on the floor scan screen, feeding the same reconciliation pipeline as keyboard-emulated input) and AC 5 (exact per-unit duplicate-scan rejection, not a fuzzy quantity-counter approximation) were previously specified but unbuilt — see `specs/00-steering/revision-log.md`'s "Camera scanner wiring and per-unit WRR label duplicate detection completed" entry (2026-08-11). Both are now implemented, tested (unit + real-Postgres integration), and reviewed (`db-migration-verifier`, `rbac-rls-reviewer`, `design-system-auditor`, `offline-sync-reviewer`, all passing). AC 3/4 were already built prior to this pass and are unaffected.
