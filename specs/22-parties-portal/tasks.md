...
- [x] **`shipment_labels.generate`/`wrr_advance_notices` RLS — verified.** `02-rbac-roles` design.md §3.2/§7.2/§7.4/§7.4a's three-condition WITH CHECK policy has been through two `rbac-rls-reviewer` design-review rounds (round 1: six real gaps found — flow_type column missing, hybrid-party bypass, capability substitution, unscoped item_id, undocumented helper-recursion risk, undesigned back-office write path; round 2 confirmed four of six fixes sound and found two further gaps — a missing `REVOKE EXECUTE FROM PUBLIC` on the new helper and two missing adversarial test bullets — both closed directly) and three real-Postgres `db-migration-verifier` passes (all 7 test bullets in `02` §13's `wrr_advance_notices` list now checked PASS, including the hybrid-party exclusion, cross-party IDOR, item-reachability, capability-substitution regression, the Supplies CHECK/NOT-NULL pair, and the `party_has_any_role` RLS-bypass property; two further real bugs found and fixed along the way — missing `GRANT USAGE ON SCHEMA public`, missing `flow_type::text` cast). This is the verification this bullet and the one above it originally asked for. **Not yet re-run by `rbac-rls-reviewer` specifically**: the narrow mechanical fixes from round 2 (the `REVOKE` statement, the two test bullets, the §7.4a owning-role note) were applied directly rather than re-submitted for a third review round — low-risk, mirrors an already-verified pattern from the other four helpers, but flagged here rather than silently assumed clean.
...
- [ ] All applicable tests pass, including real-Postgres testing, for the parts of this feature beyond `shipment_labels.generate`/`wrr_advance_notices` — most of R1–R10 has no test coverage yet and depends on `10`/`12`/`13`/`14`/`16`, all still `Draft`, `12`/`13` explicitly flagged unstable per `revision-log.md`.
