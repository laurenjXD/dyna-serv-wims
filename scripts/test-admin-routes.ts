...
import { userProfiles, userRoles, roles, rolePermissions, permissions, userPartyScopes } from '../lib/db/schema';
...
async function testAdminRoutes() {
  ...
  for (const r of routes) {
    ...
    console.log(`${r.name.padEnd(35)} [${r.perm.padEnd(25)}]: ${res.kind === 'authorized' ? '✅ AUTHORIZED' : '❌ ' + res.kind}`);
  }
}
