...
This amendment adds a lower-friction receiving mode for a line whose declared cartons/pallets may be stored separately. It deliberately does **not** change the unique payload of any printed QR label and does not treat a single scan as proof that every unique label was individually reconciled. The 2026-08-23 exact-box amendment makes every label stable across reprints and persists each numbered box slot's final location in `inventory_units` for outbound physical control.

1. One accepted label scan may open the line's batch-allocation surface. The operator sees one numbered allocation slot per `expected_qty`; slot N maps to the stable QR identity derived from `(wrr_item_id, N)`, allowing its exact final location to be retained without requiring every label to be rescanned during batch receipt.
...
3. The server persists grouped staged quantities as `wrr_item_putaway_allocations` and rechecks them on every mutation. At commit it additionally inserts one `inventory_units` identity/location row per numbered slot. The sum of allocation quantities must equal `expected_qty`; all quantities must be positive; each selected location must be active storage; and the proposed CBM must fit every selected location. A missing, stale, over-capacity, or wrong-type allocation is rejected without posting inventory.
