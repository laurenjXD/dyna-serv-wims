...
- [x] **Resolved 2026-08-06** — `02-rbac-roles`'s approved capability catalog contains `vmi_statements.read` (`assigned_party`) and its documented RLS pattern; the result is recorded in `specs/00-steering/revision-log.md`.
- [x] **Resolved 2026-08-06** — `02-rbac-roles`'s approved capability catalog contains `reporting.read` (`assigned_party`) and its documented RLS pattern; the result is recorded in `specs/00-steering/revision-log.md`.
- [x] **Updated 2026-08-06** — `02-rbac-roles`'s `shipment_labels.generate` (`assigned_party`) capability and RLS pattern passed the documented RBAC review and live Postgres contract verification; the result is recorded in `specs/00-steering/revision-log.md`.
- [x] **Updated 2026-08-06** — `01-core-data-model`'s `wrr_advance_notices` schema amendment passed the live Postgres contract verification; the result is recorded in `specs/00-steering/revision-log.md`.
...
- [x] **Updated 2026-08-06** — `18-barcode-integration` is Approved across all three documents for the 1D/linear barcode decoding exception in requirements.md FR-2.3 (`design.md` §7c; `requirements.md` R11.11(d)).
...
Testing: Unit tests for thin-form validation, non-authoritative-quantity UI framing, and the hybrid-party exclusion; real-Postgres RLS integration for the `wrr_advance_notices` write path (four-condition WITH CHECK: capability/scope, non-hybrid inbound-supplying `party_roles` via `party_has_any_role`, `item_id`/`party_visible_items` reachability, and submitter identity binding) completed for the written contract; Playwright end-to-end flow (supplier submits, back office confirms into a `wrr_items` line, floor scan matches it); manual QA confirming UI copy never implies the declared quantity is authoritative.
...
- [x] `05-ui-shell-and-navigation` has completed its own review/testing/sign-off for the `"party"` `ShellSurface` value and this portal's route-inventory entry (resolved direction, 2026-08-06, reflected in `design.md` §3).
...
- [x] `01-core-data-model`'s `flow_type`/`wrr_advance_notices` amendment and `02`'s capability rows are reconciled and verified; `07-incoming-receiving` and `18-barcode-integration` have completed their documented approval/sign-off process for the R11 additions.
...
This specification is Approved for implementation planning and contract consumption. Its implementation tasks remain gated on the explicitly named downstream contracts (`03`, `04`, `10`, `12`, `14`, and `16`) and on post-approval unit, integration, E2E, and manual testing; approval does not imply those implementation activities are complete.
