...
**Implementation status (added 2026-08-11):** `MobileQRScanner`/`ReceivingCameraScanner` (the components implementing this section) were built but, until this pass, never rendered on `07-incoming-receiving`'s floor scan screen — a phone camera could not actually be used to scan a WRR despite every underlying piece existing. Now wired in as a second scan-input method feeding the same `recordScan` server action the keyboard-emulated-scanner input uses (no parallel pipeline), positioned as the clearly secondary/alternate action per §3's one-primary-action rule. See `specs/00-steering/revision-log.md`'s 2026-08-11 completion entry.
...
**Implementation status (added 2026-08-11):** the label-generation/printing UI (this section) shipped first; the actual duplicate-detection mechanism it exists to enable — persisting which `unit_id`s have already been scanned and rejecting a repeat before it reaches the ordinary quantity-counter logic — was a genuine gap until this pass (previously, scanning the same physical label twice silently counted twice, equivalent to printing `N` identical labels, the exact outcome this section's payload design was written to prevent). Now closed: `01-core-data-model` design.md gains a `wrr_item_unit_scans` table (`(wrr_item_id, unit_id)` UNIQUE, RLS-gated on `receiving.scan`) as the persistence layer; `07`'s scan matcher (`lib/receiving/scan-matcher.ts`) checks a scanned `unit_id` against that table's already-recorded set before falling through to quantity-state checks, returning a new `duplicate_unit_scan` rejection reason that takes priority over `fully_scanned`/`over_quantity`; `07`'s `recordScan` action persists the row transactionally alongside the existing `scannedQty` increment. Verified on real Postgres, including a genuine concurrent-double-submit test proving the DB uniqueness constraint — not application-layer timing — is what prevents the double-count. See `specs/00-steering/revision-log.md`'s 2026-08-11 completion entry.
