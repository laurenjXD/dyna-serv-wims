  ...
  return (
    <div className="mx-auto w-full max-w-5xl pb-28">
      ...
      <div className="space-y-4">
        {lines.map((line, index) => <section key={line.id} className="overflow-hidden rounded-2xl border border-outline-variant/50 bg-surface-white shadow-elevation-1"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="grid size-8 place-items-center rounded-lg bg-brand-navy font-label text-body-md text-surface-white">{index + 1}</span><p className="font-mono text-mono-lg font-bold text-on-surface">{line.itemCode}</p></div><p className="mt-2 font-body text-body-md text-text-grey">{line.itemDescription ?? line.itemCode}</p><div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 font-body text-body-sm text-text-grey sm:grid-cols-4"><p><span className="font-label uppercase tracking-wide">Cust PN</span><br /><span className="font-mono text-on-surface">{line.customerItemCode ?? "—"}</span></p><p><span className="font-label uppercase tracking-wide">Lot</span><br /><span className="font-mono text-on-surface">{line.lotNumber}</span></p><p><span className="font-label uppercase tracking-wide">Qty</span><br /><span className="font-mono text-on-surface">{line.qty}</span></p><p><span className="font-label uppercase tracking-wide">SPQ</span><br /><span className="font-mono text-on-surface">{line.spq}</span></p></div></div><div className="rounded-xl bg-brand-navy px-4 py-3 text-surface-white"><p className="font-label text-body-md uppercase tracking-wide">Stage {line.numberOfBoxes} boxes</p><p className="mt-1 flex items-center gap-1.5 font-body text-body-md"><MapPin size={18} aria-hidden="true" /> {line.locationLabel}</p></div></div></section>)}
      </div>
