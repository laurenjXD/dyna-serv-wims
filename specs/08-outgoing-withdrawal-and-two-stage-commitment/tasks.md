...
- [ ] Resolve partial pick, shortage, damage, cancellation, expiry, reversal, and reallocation behavior before implementation.
  ...
  - [x] **Cancellation/release flow:** No cancellation UI in v1. Pick lists that are not executed expire automatically only. The `cancelled`/`released` status values remain in the schema but no application route, server action, or capability check for triggering cancellation is implemented until this is revisited. Floor user sees "Pick list no longer active — return to queue" when expiry is hit at Stage 2. *(Resolved 2026-08-09. See revision-log.md.)*
  - [x] **Expiry enforcement mechanism:** Dual enforcement — Stage 2 dispatch rejects and marks `expired` in real time when `expires_at` is past; a nightly pg_cron sweep marks any unattempted commitments past `expires_at` as `expired` and decrements `qty_committed` on `lot_location_balances`. CRON is the safety net; Stage 2 check is the primary enforcer. *(Resolved 2026-08-09. See revision-log.md.)*
