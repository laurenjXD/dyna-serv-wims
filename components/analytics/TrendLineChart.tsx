...
export function TrendLineChart({ data, period, onPeriodChange, label, title, color = "#475569" }: {
  ...
  return <section className="rounded bg-surface-white p-6 shadow-elevation-1">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><span className="font-body text-body-sm text-text-grey">{label}</span><div className="flex gap-2" role="tablist" aria-label={`${title} period`}>
      {(["day", "week", "month"] as const).map((value) => <button key={value} type="button" role="tab" aria-selected={period === value} onClick={() => onPeriodChange(value)} className={`rounded px-3 py-1 font-label text-label font-semibold ${period === value ? "bg-brand-red text-surface-white" : "bg-surface-light-grey text-on-surface"}`}>{value}</button>)}
    </div></div>
