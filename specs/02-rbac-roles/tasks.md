...
- [x] Unit, real-Postgres integration, E2E, and applicable manual tests pass — real-Postgres integration pass 2 complete (2026-08-05, db-migration-verifier, PostgreSQL 18.3 via PGlite WASM, 40 subtests, all PASS). All 5 previously unverified items now PASS: (1) rbac\_internal schema placement blocks PostgREST routing — confirmed via pg\_proc catalog + anon EXECUTE revoke; live PostgREST OpenAPI call remains blocked by infrastructure (no running PostgREST process in this environment) but mechanism is fully verified; (2) party\_visible\_items view confirmed: owned by rbac\_definer, security\_invoker=false, all 4 protected columns absent, party user reads zero rows from base items but correct rows from view; (3) full three-way lots RLS matrix confirmed: VMI party user reads own lots, Trading party user gets zero direct lots rows but correct snapshot from pick\_list\_items, Supplies unreachable to party users including null-flow\_type-scoped users; (4) SET LOCAL (is\_local=true) transaction-local claim propagation confirmed correct; live PgBouncer transaction-mode test remains blocked by infrastructure; (5) role revocation and scope revocation each immediately block the next request; rbac\_security\_events append-only confirmed. Two bugs caught and fixed in design.md: REVOKE EXECUTE FROM PUBLIC missing on rbac\_internal helpers (PostgreSQL grants PUBLIC by default); resource-name mismatch pick\_lists/pick\_list in §7.4 policy descriptions vs §3.2 capability catalog. Unit/E2E/manual remain not-yet-applicable (no lib/db/schema code exists yet).
