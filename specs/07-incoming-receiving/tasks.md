...
- [x] **Added 2026-08-10, reopened and re-scoped to per-unit 2026-08-20**: after a `store`-disposition unit's scan matches its expected line, display the system-suggested location(s) (design.md §6.2/§6.2a) before that unit's "Store" action; allow accept-or-override. The 2026-08-10 item described this happening once for the whole line's full scanned quantity; it now happens once per individually scanned unit, and the suggestion list must show every eligible candidate location, not one recommendation. *(Implemented 2026-08-21: `PutawayLocationSelector` renders all candidates, `suggestPutawayLocations` called with `requestedQty: 1` per unit.)*
- [x] **Added 2026-08-20**: rework the floor scan UI so the "Store"/"Hold" commit UI appears — and is actionable — as soon as a single unit is scanned, not gated behind `scannedQty >= expectedQty` for the whole line. `app/(authenticated)/receiving/[wrrId]/receive/page.tsx` currently gates this behind full-line scan completion; this must change to per-unit. *(Implemented 2026-08-21: `primaryReadyLine` now resolves from the `wrrItemId` search param carried through the scan-success redirect, guarded by `committedAt === null`; `readyToCommit`'s whole-line fallback is now scoped to inspect-disposition lines only, since it's unreachable for store lines.)*
...
- [x] Recheck WRR state, scan totals, conformance, active references, flow partition, lot metadata, and required prerequisites inside the transaction — **per-unit for `store` lines, per-line for `inspect` lines** (2026-08-10, re-scoped 2026-08-20).
- [x] Create approved lots/available state and immutable receiving transactions atomically, **per unit for `store` lines** (lot created once per line, on the first committed unit, and reused for every subsequent unit — design.md §9), **per line for `inspect` lines** (2026-08-10, re-scoped 2026-08-20).
- [x] **Reopened 2026-08-10, re-scoped 2026-08-20**: transition each `store`-disposition unit to a terminal committed state independently (each `inspect`-disposition line remains a single terminal-state transition, unchanged); transition the parent line to its own terminal committed state only once every one of its expected units is terminal; transition the WRR itself to `confirmed` only once every line has reached that state; return the authoritative result for duplicate retries per unit-commit event (per line for `inspect`).
- [x] Ensure a failed `store`-disposition unit commit rolls back completely for that unit and remains recoverable, without affecting any other unit's — on the same line or any other line's — already-committed state (2026-08-10, re-scoped 2026-08-20 from "per-line" to "per-unit" for `store` lines). *(Verified 2026-08-21 against real Postgres, including two concurrency races found and fixed during verification — see revision-log.md.)*
- [x] **Reopened 2026-08-10, moves earlier in the flow; re-scoped 2026-08-20**: integrate the putaway recommendation/suggestion at each unit's scan time (before that unit's "Store" commit), listing every eligible candidate location (design.md §6.2a) rather than one recommendation, using approved locations/capacity interfaces without duplicating location logic — not only as a post-commit recommendation, and not batched once per line.
- [x] **Added 2026-08-20**: implement the create-or-increment `lot_location_balances` write per committed unit (design.md §9 step 4) — insert a new row if this lot has no existing balance row at the chosen location yet, otherwise increment the existing row's `qty_received`/`qty_remaining` by one unit. This is the mechanism that makes design.md §6.2b's multi-location split work; no `01-core-data-model` migration is needed for it.
- [x] **Added 2026-08-20**: re-scope the per-unit idempotency mechanism (design.md §9) — `wrr_items.committed_at` now marks only the line's full terminal completion (all expected units committed), not a single line-level commit event; each individual unit-commit's own idempotency uses the general command idempotency-key mechanism (design.md §4), not `committed_at`. Confirm the existing `wrr_items_protect_committed_at` trigger (migration `0022_receiving_inventory_insert_policies.sql`) still correctly protects this re-scoped meaning (it should, since it was already a one-time `NULL → non-NULL` guard). *(Implemented 2026-08-21: `commitStoreUnit`'s per-unit idempotency uses a deterministic `transaction_number` derived from `sha256(wrrItemId:idempotencyKey)`, checked against `inventory_transactions` before any write.)*
- [x] Record completed putaway through the owning inventory transaction boundary.
- [x] **Added 2026-08-10**: implement the flow-type cross-check rejection (design.md §6.1) — a scanned item's `items.flow_type` must match the WRR's `wrr_documents.flow_type`, rejected through the existing wrong-item exception path.
- [x] **Added 2026-08-10**: implement the location-first "Hold" sequence for `inspect`-disposition lines (design.md §6.3) — confirm inspection location, then scan, then commit. Unaffected by the 2026-08-20 per-unit amendment (still whole-line). *(Verified 2026-08-21: `recordScan`'s scanned_qty increment for inspect lines — the mechanism this whole-line gate depends on — was found broken by an over-broad edit during the per-unit amendment, then fixed and proven end-to-end against real Postgres. See revision-log.md.)*
...
**2026-08-20 amendment implementation — completed 2026-08-21.** All files below were reworked, RED→GREEN→VERIFY per this repo's TDD protocol, and confirmed against real Postgres (see `specs/00-steering/revision-log.md`'s 2026-08-21 entry for the full account, including two real concurrency bugs found and fixed during verification and one cross-cutting regression — inspect-disposition scanning silently broken by an over-broad edit — caught and fixed before sign-off):

- `lib/receiving/commit-validation.ts` — `validateLineCommit` now validates one unit's readiness at a time for `store` lines (`scannedQty < expectedQty` accepted, `>= expectedQty` rejected as "already fully committed"); `inspect` lines unchanged.
- `lib/actions/receiving.ts`'s `commitWrrLine` — dispatches to `commitStoreUnit` (one unit per call, `idempotencyKey` required) or the unchanged `commitInspectLine` (whole-line) by disposition. `recordScan` no longer writes `scanned_qty` for `store` lines (that column now means "committed so far" there) but still does for `inspect` lines (unchanged meaning, restored after being incorrectly removed during the same pass).
- `app/(authenticated)/receiving/[wrrId]/receive/page.tsx` — shows the Store/Hold action per unit via a `wrrItemId` search param carried through the scan-success redirect (guarded by `committedAt === null` against a stale/replayed URL); per-unit vs. per-line progress and commit-success copy is disposition-aware.
- `lib/db/queries/locations.ts`'s `suggestPutawayLocations` — contract unchanged, confirmed already correct: called with `requestedQty: 1` per unit, and naturally reflects capacity already consumed by earlier units of the same line since each unit-commit is a completed transaction before the next call.
- `lib/receiving/scan-matcher.ts` — unchanged, as expected; the per-unit Hold override (§6.4) referenced here is still explicitly blocked on `01`, see the unresolved item above.
...
- [x] Receipt commit precondition and idempotency result handling. **Re-scoped 2026-08-20**: for `store` lines, this now means per-unit precondition/idempotency handling (one unit's commit at a time), plus the create-or-increment `lot_location_balances` logic (design.md §9); `inspect` lines keep the existing per-line test shape. *(45/45 unit tests passing as of 2026-08-21 — `lib/receiving/__tests__/commit-validation.test.ts`, `lib/actions/__tests__/receiving.test.ts`.)*
...
- [x] Verify authorized receipt commit atomically creates the approved lots and immutable receiving transactions exactly once. **Re-scoped 2026-08-20**: for `store` lines, verify this holds per unit (one lot created on the first unit, reused thereafter; one `lot_location_balances` row created or incremented per unit's location; one `inventory_transactions` row per unit) and verify a line whose units split across more than one location ends up with the correct multiple `lot_location_balances` rows for one lot. *(Verified 2026-08-21 — 17/17 real-Postgres integration tests, `lib/actions/__tests__/receiving.commit-line.integration.test.ts`.)*
- [x] Verify failed commit rolls back and duplicate retries return one authoritative outcome. **Re-scoped 2026-08-20**: for `store` lines, verify this at unit-commit granularity — a failed or retried unit-commit does not affect any other already-committed unit, on the same line or a different one. *(Verified 2026-08-21, including two genuine concurrency races found and fixed during this verification: identical-`idempotencyKey` double-submit and distinct-`idempotencyKey` concurrent commits on the same line — see revision-log.md. Stress-tested to 9-10 concurrent calls with no unhandled rejections, no lock contention issue observed.)*
