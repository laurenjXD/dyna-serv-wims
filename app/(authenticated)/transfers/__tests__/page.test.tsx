// RED test — TransferListPage (app/(authenticated)/transfers/page.tsx) must
// become a superseded entry point that forwards straight to the merged
...
//   `redirect()` to the real destination, preserving query params; detail/
//   action sub-routes such as /transfers/new, /transfers/[transferId],
//   /transfers/[transferId]/execute, /transfers/[transferId]/inspect stay
//   real and are NOT touched by this change).
//
// Current state (pre-fix): `../page.tsx` still renders its own real
// floor/office transfer-queue list (permission gate, DB query, JSX) instead
// of redirecting. This test mocks `next/navigation`'s `redirect` (which
// throws in real Next.js — mocked here as a no-op spy per this repo's RED
// convention for page-redirect tests) plus this page's existing auth/DB
// dependencies (so the *current*, still-real implementation can run to
// completion without crashing on missing request context), then asserts
...
// implementation renders successfully (floor branch, empty lists) rather
...
vi.mock("@/lib/db/queries/transfers", () => ({
  listTransferRequests: vi.fn(async () => ({ rows: [], total: 0 })),
  listInspectionCases: vi.fn(async () => ({ rows: [] })),
}));

describe("TransferListPage (app/(authenticated)/transfers/page.tsx)", () => {
  beforeEach(() => {
  ...
  // 2026-08-17 sidebar/IA decision): `/transfers` retires as a top-level
  ...
  it("AC ui-implementation-plan.md P4: redirects to /inventory?tab=inspection", async () => {
    const { default: TransferListPage } = await import("../page");

    await TransferListPage({ searchParams: Promise.resolve({}) });
