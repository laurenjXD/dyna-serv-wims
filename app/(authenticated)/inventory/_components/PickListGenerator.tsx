...
export function PickListGenerator({ itemId, flowType, organizationId, lots, action }: {
  ...
  return <div>
    {!open && <button type="button" onClick={() => setOpen(true)} className="inline-flex h-11 items-center gap-2 rounded bg-primary px-4 font-label text-label text-surface-white">Generate Pick List</button>}
    {open && <form action={action} className="mt-3 grid gap-3 rounded-lg border border-outline-variant/30 bg-surface-white p-4">
      ...
      <div className="flex gap-2"><button type="submit" disabled={!request} className="h-11 rounded bg-primary px-4 font-label text-label text-surface-white disabled:opacity-50">Generate Pick List</button><button type="button" onClick={() => setOpen(false)} className="h-11 rounded border px-4 font-label text-label">Cancel</button></div>
    </form>}
  </div>;
}
